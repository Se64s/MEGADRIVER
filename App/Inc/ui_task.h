/**
  ******************************************************************************
  * @file           : ui_task.h
  * @brief          : Task to handle ui actions
  ******************************************************************************
  */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __UI_TASK_H
#define __UI_TASK_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/

#include <stdint.h>
#include <stdbool.h>

#include "sys_rtos.h"

/* Private includes ----------------------------------------------------------*/
/* Private defines -----------------------------------------------------------*/

/* Task parameters */
#define UI_TASK_NAME                    "UI"
#define UI_TASK_STACK                   ( 256U )
#define UI_TASK_PRIO                    ( 1U )

/* Enable verbose output */
// #define UI_DBG_VERBOSE

/* UI signals */
#define UI_SIGNAL_ENC_UPDATE_CCW        ( 1UL << 0U )
#define UI_SIGNAL_ENC_UPDATE_CW         ( 1UL << 1U )
#define UI_SIGNAL_ENC_UPDATE_SW_SET     ( 1UL << 2U )
#define UI_SIGNAL_ENC_UPDATE_SW_RESET   ( 1UL << 3U )
#define UI_SIGNAL_SCREEN_UPDATE         ( 1UL << 4U )
#define UI_SIGNAL_IDLE                  ( 1UL << 5U )
#define UI_SIGNAL_RESTORE_IDLE          ( 1UL << 6U )
#define UI_SIGNAL_MIDI_CC               ( 1UL << 7U )
#define UI_SIGNAL_RESTORE_CC            ( 1UL << 8U )
#define UI_SIGNAL_ERROR                 ( 1UL << 9U )
#define UI_SIGNAL_CMD                   ( 1UL << 10U )
#define UI_SIGNAL_NOT_DEF               ( 1UL << 31U )
#define UI_SIGNAL_ALL                   ( 0xFFFFFFFFU )

/* Exported types ------------------------------------------------------------*/

/** Ui Cmd Type definition */
typedef enum
{
    UI_CMD_CC_UPDATE = 0x00U,
    UI_CMD_NOT_DEF = 0xFFU,
} UiCmdType_t;

/** Ui command voice update payload */
typedef struct
{
    uint8_t u8CcId;
    uint8_t u8Data;
} UiCmdCcUpdatePayload_t;

/** Union definitions with all event payload */
typedef union
{
    UiCmdCcUpdatePayload_t xCcUpdate;
} UiCmdPayload_t;

/** Midi taks cmd definition */
typedef struct
{
    UiCmdType_t eCmd;
    UiCmdPayload_t uPayload;
} UiCmd_t;

/* Exported constants --------------------------------------------------------*/
/* Exported macro ------------------------------------------------------------*/
/* Exported functions prototypes ---------------------------------------------*/

/**
  * @brief Init resources for UI tasks
  */
void vUiTaskInit(void);

/**
  * @brief Notify event to a task.
  * @param u32Event event to notify.
  * @retval operation result, true for correct read, false for error
  */
bool bUiTaskNotify(uint32_t u32Event);

/**
  * @brief Send a command to ui task.
  * @param xUiTaskCmd Command to send.
  * @return true cmd queue.
  * @return false cmd not queue.
  */
bool bUiSendCmd(UiCmd_t xUiTaskCmd);

#ifdef __cplusplus
}
#endif

#endif /* __UI_TASK_H */

/*****END OF FILE****/
